...
public class HdfsTest {
	public static void main(String[] args) {
		 ...
		 * 也可以基于持久化SparkStreamingContext的内容 来恢复过来(典型的场景是Driver崩溃后重新启动，由于Spark
		 ...
		 * 之前需要把前面正在运行的SparkStreamingContext对象关闭掉，由此，我们获得一个重大启发：
		 * SparkStreamingContext 是Spark core上的一个应用程序而已，只不过Spark
		 * Streaming框架箱运行的话需要Spark工程师写业务逻辑
		 */
		...
		/**
		 * 可以从失败中恢复Driver，不过还需要制定Driver这个进程运行在Cluster，并且提交应用程序的时候 指定
		 * --supervise;
		 */
		JavaStreamingContext jsc = JavaStreamingContext.getOrCreate(checkpointDirectory, factory);
		/**
		 * 现在是监控一个文件系统的目录 此处没有Receiver，Spark
		 * Streaming应用程序只是按照时间间隔监控目录下每个Batch新增的内容(把新增的) 作为RDD的数据来源生成原始的RDD
		 */
		// 指定从HDFS中监控的目录
		JavaDStream<String> lines = jsc.textFileStream("hdfs://Master:9000/library/SparkStreaming/Data");
		/**
		 * 第四步：接下来就像对于RDD编程一样基于DStreaming进行编程！！！ 原因是： DStreaming是RDD产生的模板（或者说类）。
		...
		/**
		 * 第4.2步：对初始的JavaRDD进行Transformation级别的处理，例如map，filter等高阶函数等的编程，
		 * 来进行具体的数据计算 在4.1的基础上，在单词拆分的基础上对每个单词实例计数为1，也就是word => (word,1)
		 */
		 ...
		 * 触发真正的Job运行是基于设置的Duration时间间隔的 一定要注意的是：Spark
		 * Streaming应用程序要想执行具体的Job，对DStream就必须有output Stream操作， output
		 * Stream有很多类型的函数触发，例如：print，saveAsTextFile,saveAsHadoopFiles等，
		 * 其实最为重要的一个方法是 foraeachRDD，因为Spark
		 * Streaming处理的结果一般都会放在Redis，DB，DashBoard等上面，foreachRDD主要就是用来完成这些
